...
def detect_occupancy(video_path, map_path, weights="yolov8n.pt",
                     ...
                     display=False, save_image=True):

    # Ensure output directories exist
    os.makedirs("test", exist_ok=True)
    os.makedirs("test/results", exist_ok=True)
    os.makedirs("test/results/images", exist_ok=True)
    os.makedirs("test/results/data", exist_ok=True)
    os.makedirs("test/results/grid_results", exist_ok=True)
    ...
    while True:
        ...
        # Count zones by type for grid analysis
        ...
        # Different display for grid analysis vs regular detection
        ...
        if save_image:
            ...
            # Determine if this is a grid photo or regular input
            if "grid_photos" in video_path:
                # Grid analysis: always use the same filename (overwrite previous)
                output_path = f"test/results/grid_results/{input_basename}_analysis.png"
            else:
                # Regular detection: use unique filenames
                base_name = f"test/results/images/{input_basename}_result"
